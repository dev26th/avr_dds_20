...
const uint8_t  sine0[] __attribute__ ((section (".MySection7")))= //sine 256 values, start from 0
{
...
void Menu_Update(uint8_t on)
{
	...
	if((SG.mode >= FIRST_SIGNAL && SG.mode <= LAST_SIGNAL) || SG.mode == MODE_SWEEP)
		{
			CopyStringtoLCD(CLR, 0, 1 );
			LCDGotoXY(0, 1);
